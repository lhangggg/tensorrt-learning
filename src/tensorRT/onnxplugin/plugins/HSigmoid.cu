...
static __global__ void hsigmoid_kernel_fp32(float *input, float *output,
                                            int edge) {
...
// static __global__ void hsigmoid_kernel_fp16(__half* input, __half* output,
// int edge) {
...
public:
  ...
  virtual void config_finish() override {
    ...
    // 		INFO("Weight[%d] shape is %s, dtype = %s, value[0] = %f", i,
    // w->shape_string(), data_type_string(w->type()), float(w->at<__half>(0)));
    // 	}else{
    // 		INFO("Weight[%d] shape is %s, dtype = %s, value[0] = %f", i,
    // w->shape_string(), data_type_string(w->type()), w->at<float>(0));
    // 	}
    // }
  }
  ...
  virtual std::shared_ptr<LayerConfig> new_config() override {
    ...
    // cfg->support_dtype_set_ = {nvinfer1::DataType::kHALF,
    // nvinfer1::DataType::kFLOAT};
    cfg->support_dtype_set_ = {nvinfer1::DataType::kFLOAT};
    return cfg;
  }

  virtual nvinfer1::DimsExprs
  getOutputDimensions(int32_t outputIndex, const nvinfer1::DimsExprs *inputs,
                      int32_t nbInputs,
  ...
  int enqueue(const std::vector<GTensor> &inputs, std::vector<GTensor> &outputs,
              const std::vector<GTensor> &weights, void *workspace,
    ...
    if (config_->usage_dtype_ == TRT::DataType::Float) {
      hsigmoid_kernel_fp32<<<grid, block, 0, stream>>>(
          inputs[0].ptr<float>(), outputs[0].ptr<float>(), count);
    } else if (config_->usage_dtype_ == TRT::DataType::Float16) {
      // hsigmoid_kernel_fp16 <<<grid, block, 0, stream >>>
      // (inputs[0].ptr<__half>(), outputs[0].ptr<__half>(), count);
