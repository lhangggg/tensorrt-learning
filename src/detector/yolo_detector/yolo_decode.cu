...
static __device__ void affine_project(float *matrix, float x, float y,
                                      float *ox, float *oy)
{
...
static __global__ void nms_kernel(float *bboxes, int max_objects,
                                  float threshold)
{
                        ...
                        cudaStream_t stream)
{
    ...
    checkCudaKernel(nms_kernel<<<grid, block, 0, stream>>>(parray, max_objects,
                                                           nms_threshold));
}
} // namespace yolo
} // namespace trt