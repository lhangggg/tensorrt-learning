...
static float iou(const Box &a, const Box &b)
{
    ...
    float c_area =
        std::max(cright - cleft, 0.0f) * std::max(cbottom - ctop, 0.0f);
    if (c_area == 0.0f)
        return 0.0f;
...
using ControllerImpl = InferController<cv::Mat,                 // input
                                       BoxArray,                // output
                                       tuple<std::string, int>, // start param
...
public:
    ...
    virtual bool startup(const std::string &file, Type type, int gpuid,
    ...
    virtual void worker(std::promise<bool> &result) override
    {
        ...
        std::string file = get<0>(start_param_);
        ...
        while (get_jobs_and_wait(fetch_jobs, max_batch_size))
        {
            ...
            for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
            {
                ...
                affin_matrix_device.copy_from_gpu(
                    affin_matrix_device.offset(ibatch),
                    mono->get_workspace()->gpu(), 6);
                input->copy_from_gpu(input->offset(ibatch), mono->gpu(),
                                     mono->count());
            ...
            for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
            {
                ...
                {
                    image_based_boxes =
                        cpu_nms(image_based_boxes, nms_threshold_);
                }
    ...
    virtual bool preprocess(Job &job, const cv::Mat &image) override
    {
        ...
        // checkCudaRuntime(cudaMemcpyAsync(image_host,   image.data,
        // size_image, cudaMemcpyHostToHost,   stream_));
        //  speed up
        memcpy(image_host, image.data, size_image);
        memcpy(affine_matrix_host, job.additional.d2i,
               sizeof(job.additional.d2i));
        checkCudaRuntime(cudaMemcpyAsync(image_device, image_host, size_image,
                                         cudaMemcpyHostToDevice,
                                         preprocess_stream));
        checkCudaRuntime(
            cudaMemcpyAsync(affine_matrix_device, affine_matrix_host,
    ...
    virtual std::vector<shared_future<BoxArray>>
...
std::shared_ptr<Infer> create_infer(const std::string &engine_file, Type type,
                                    int gpuid, float confidence_threshold,
                                    float nms_threshold, NMSMethod nms_method,
                                    int max_objects,
                                    bool use_multi_preprocess_stream)
{
                     ...
                     Type type, int ibatch)
{
    ...
    if (type == Type::V5 || type == Type::V3 || type == Type::V7)
    {
        normalize = CUDAKernel::Norm::alpha_beta(
            1 / 255.0f, 0.0f, CUDAKernel::ChannelType::Invert);
    }
    else if (type == Type::X)
    {
