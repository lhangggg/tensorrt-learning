...
static void inference_and_performance(int deviceid, const std::string &engine_file, TRT::Mode mode,
                                      yolo::Type type, const std::string &model_name)
{
    ...
    float inference_average_time =
        (iLogger::timestamp_now_float() - begin_timer) / ntest / images.size();
    auto type_name = yolo::type_name(type);
    auto mode_name = TRT::mode_string(mode);
    INFO("%s[%s] average: %.2f ms / image, FPS: %.2f", engine_file.c_str(), type_name,
         inference_average_time, 1000 / inference_average_time);
    append_to_file("perf.result.log", iLogger::format("%s,%s,%s,%f", model_name.c_str(), type_name,
                                                      mode_name, inference_average_time));

    std::string root = iLogger::format("%s_%s_%s_result", model_name.c_str(), type_name, mode_name);
    iLogger::rmtree(root);
    iLogger::mkdir(root);
    ...
    for (int i = 0; i < boxes_array.size(); ++i)
    {
        ...
        for (auto &obj : boxes)
        {
            ...
            cv::putText(image, caption, cv::Point(obj.left, obj.top - 5), 0, 1, cv::Scalar::all(0),
                        2, 16);
        }

        std::string file_name = iLogger::file_name(files[i], false);
        std::string save_path = iLogger::format("%s/%s.jpg", root.c_str(), file_name.c_str());
        INFO("Save to %s, %d object, average time %.2f ms", save_path.c_str(), boxes.size(),
...
static void test(yolo::Type type, TRT::Mode mode, const std::string &model)
{
...
void multi_gpu_test()
{
    ...
    auto multi_gpu_infer =
        yolo::create_multi_gpu_infer("yolov5s-6.0.FP32.trtmodel", yolo::Type::V5, devices);
...
void YoloDetector::init(const std::string &model_path)
{
    std::cout << "yolo init" << std::endl;
    return;
}

void YoloDetector::forward()
{
    std::cout << "yolo foreard" << std::endl;
    return;
}
